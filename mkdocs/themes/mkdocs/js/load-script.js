...
function checkDeps(net_path, ftype, noInject) {
  ...
  if (allFiles.indexOf(full_net_path) >= 0 || allFiles.indexOf(net_path) >= 0) {
    ...
    if (noInject) {
      ...
      if (ftype == 'css') {
          window.webInject.css(window.location.origin + '/' + net_path, function(){console.log(net_path + ' injected.')})
      } else if (ftype == 'js' || ftype == 'javascript') {
          window.webInject.js(window.location.origin + '/' + net_path, function(){console.log(net_path + ' injected.')})
      }
    }
  }
};
